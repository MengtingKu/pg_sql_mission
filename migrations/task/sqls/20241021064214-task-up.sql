...
SELECT 
    ...
    b.id AS skill_id
FROM "COACH" a
CROSS JOIN (SELECT id FROM "SKILL" WHERE name = '重訓') b;
...
WHERE b.email = 'muscle@hexschooltest.io') c
CROSS JOIN (SELECT id FROM "SKILL" WHERE name = '瑜伽') d;
...
FROM "COACH" a
INNER JOIN  "USER" b ON a.user_id = b.id
WHERE b.email = 'starplatinum@hexschooltest.io') c
CROSS JOIN (SELECT id FROM "SKILL" WHERE name in ('有氧運動', '復健訓練')) d;
...
FROM "COACH_LINK_SKILL"
INNER JOIN  "SKILL" ON  "COACH_LINK_SKILL".skill_id = "SKILL".id
GROUP BY "SKILL".name ORDER BY coach_total DESC
LIMIT 1;
